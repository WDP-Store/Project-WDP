...
const ProductItem = (props) => {
  ...
  useEffect(() => {
    if (isLogged) {
      axios
        .get(
          `https://wdp.bachgiaphat.vn/wishlists?product=${product?._id}&user=${JSON.parse(localStorage.getItem("data"))?._id
          }`
        )
        ...
        >
          {isWish ? (
            ...
            <button className="border-0 bg-transparent" onClick={addToWishList}>
              <img src={whiteWish} style={{ width: "18px", height: "18px" }} alt="wishlist" />
            </button>
          )}
        </div>
        <Link to={`/product/${product._id}`} className="text-decoration-none">
          ...
          >
            ...
            <div className="w-100 d-flex justify-content-between align-items-baseline">
              <h6 className="brand text-uppercase text-secondary">{product.category.name}</h6>
