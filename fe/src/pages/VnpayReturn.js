...
const VnpayReturn = () => {
  ...
  useEffect(() => {
    if (vnpayResponseCode === '00') {
      axios
        .get(`https://wdp.bachgiaphat.vn/orders/vnpay-return${location.search}`)
        .then((res) => {
          console.log("res return");
          console.log(res);
          if (res.data.code === '00') {
            axios
              .get(`https://wdp.bachgiaphat.vn/orders/find-by-order-vnpay-id/${searchParams.get('vnp_OrderInfo')}`)
              .then((res) => {
                if (res.data) {
                  axios
